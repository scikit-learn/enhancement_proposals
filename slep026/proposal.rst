...
******************************************
SLEP026: Deprecate and Remove score Method
******************************************
...
This SLEP proposes to deprecate and to remove the ``score`` method from the
scikit-learn API, for classifiers as well as regressors. ``classifier.score`` always
uses *accuracy*, ``regressor.score`` most often uses *R2*.

2. Motivation
=============

The main motivation is to avoid the blind usage of a default score that, in most use
cases, is not a good default. This is one of the conclusions of the rejected SLEP 25
[3]_: *There is no good default score*.

The ``score`` method of estimators may be used to compare models. It is used as
default in cross validation tools, for example in ``cross_validate``.
...
Additionally, for classifiers, a user might favor to use ``predict_proba`` in her
application instead of the 50%-thresholded ``predict``, in which case a proper scoring
rule like log loss and Brier score is to be preferred from a methodological point of
view. One could say: *The default score is too easy to use*.

Instead, we would like to foster a conscious decision by users and engagement with the
subject. Some guidelines are already provided in the user guide, see [2]_ subsection
"Which scoring function should I use?".

A second point is that ``score`` is redundant. The ``sklearn.metrics`` module provides
...
The comparability for classifiers is given because all classifiers inherit the accuracy
score from ``ClassifierMixin``. Most regressors inherit the R2 score from
``RegressorMixin``, there are, however, some exceptions.

The second point, purpose, is much harder and often contradicts the comparability.
...
the minority of estimators.
For model comparison and assessment, e.g. comparing a ``DecisionTreeRegressor`` with a
``Lasso`` or assessing a final selected model, it is good if the score is aligned with
the application purpose of the model. This depends heavily on the use case and the
variety of use cases is so large that a unified choice seems futile.
...
2. Using a summary score like R2 or AUC with ``LeaveOneOut``, see https://github.com/scikit-learn/scikit-learn/issues/5097.
...
4. Your application has very different costs for false positives versus false
   ...
   - ``LogisticRegressionCV().fit(X, y)``; or
...
and functions. It is written as of scikit-learn version 1.8.
...
+------------------------------------+-----------+-----------------------+-------------------------------+------------------------------------------------------------------+
|``LogisticRegressionCV``            | parameter | ``scoring``           | default accuracy              |                                                                  |
+                                    +-----------+-----------------------+-------------------------------+------------------------------------------------------------------+
...
---------------------------
In principle, this deprecation can be accomplished with the usual deprecation policy
of scikit-learn. It is, however, not backward compatible in the sense that, as with all
deprecations, future versions won't work with code written for older versions: an
``AttributeError`` is raised if ``estimator.score(X, y)`` is called, a
``ValueError`` is raised if ``cv_tool(scoring=None)`` is called, a ``TypeError`` for
``cv_tool()`` (because ``scoring`` has no default value anymore).
...
5.1 Add ``scoring`` parameter to ``score``
------------------------------------------
...
5.3 SLEP 25: Change default of ``scoring`` parameter of ``score``
-----------------------------------------------------------------
